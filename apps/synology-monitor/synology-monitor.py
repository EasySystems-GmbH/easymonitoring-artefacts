...
# directly (``python3 synology-monitor.py``). These modules ship in the install
...
from src.core.auth import cookies as _auth_cookies  # noqa: E402
...
INTERVAL_MIN = 1
INTERVAL_MAX = 1440
CHECK_MODES = ("smart", "storage", "ping", "port", "dns", "backup", "service")
...
GITHUB_REPO = os.environ.get("PUBLIC_REPO", "EasySystems-GmbH/EasySystems-GmbH/easymonitoring-artefacts")
...
# Wire the extracted auth state helpers (src/core/auth/auth_state.py) to this
# entry script's runtime data dir, product name + lockout policy.
# Behavior-preserving: the helper signatures are unchanged, so all existing call
# sites keep working.
_auth_state.configure(
    runtime_data_dir_provider=get_runtime_data_dir,
...
def _agent_apply_hosted_fleet_policy(cfg: Dict[str, Any], fleet_config: Dict[str, Any]) -> List[str]:
    ...
    synology = defaults.get("synology") if isinstance(defaults.get("synology"), dict) else {}
    notes: List[str] = []
    if "webEnabled" in synology or "web_enabled" in synology:
        cfg["web_enabled"] = bool(synology.get("webEnabled", synology.get("web_enabled", True)))
        notes.append(f"web_enabled={cfg['web_enabled']}")
    if "lockLocalUiWhenConnected" in synology or "lock_local_ui_when_connected" in synology:
        cfg["lock_local_ui_when_connected"] = bool(
            synology.get("lockLocalUiWhenConnected", synology.get("lock_local_ui_when_connected", False))
        )
        notes.append(f"lock_local_ui={cfg['lock_local_ui_when_connected']}")
    return notes


def _resolve_package_service_script() -> Path:
    for package in ("synology-monitor-agent", "synology-monitor"):
        candidate = Path(f"/var/packages/{package}/scripts/start-stop-status")
        if candidate.exists():
            return candidate
...
def _agent_apply_web_policy(cfg: Dict[str, Any]) -> str:
    script = _resolve_package_service_script()
    if not script.exists():
        return "web policy skipped (service script missing)"
    ...
    action = "start" if desired and not running else ("stop" if not desired and running else "")
    if not action:
        return "web policy unchanged"
    try:
        subprocess.run([str(script), action], capture_output=True, text=True, timeout=20, check=False)
        return f"web policy applied ({action} local UI)"
    except Exception as exc:
        return f"web policy {action} failed: {type(exc).__name__}"
...
def _peer_push_to_master(cfg: Dict[str, Any]) -> str:
    ...
    auth = _load_auth_state()  # unix-monitor parity: required before last_login_* in push_payload
...
def _peer_test_connection(url: str, token: str) -> str:
    ...
    try:
        ...
        if isinstance(e, ConnectionRefusedError) or "Connection refused" in str(e):
            host, port = _parse_peer_host_port(url, PEER_DEFAULT_PORT)
            if host:
                msg += f". {_peer_lan_reachability_hint(host, port)}"
        return msg
...
def add_monitor() -> None:
    ...
    mode = prompt_with_back("Check mode: smart / storage / ping / port / dns / backup / service", "smart")
    if mode is None:
        return
...
def _render_setup_html(
    ...
    event_labels = {"all": "All events", "smart": "Smart", "storage": "Storage", "ping": "Ping", "port": "Port", "dns": "DNS", "backup": "Backup", "service": "Service"}
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            if self.path == "/api/peer/create-monitor":
                ...
                for extra_key in ("probe_host", "probe_port", "dns_name", "dns_server", "service_names", "service_description_filter"):
            ...
            if self.path == "/peer/test-connection":
                ...
                diag_lines = [
                    ...
                    ("Next action: Run 'Sync now' to push an agent snapshot."
                     if ok else "Next action: Save settings, verify master is reachable from this NAS (port/firewall), then retry.")
                ]
                ssl_warning = self._ssl_warning_text()
            ...
            if self.path == "/peer/save-settings":
                ...
                if role == "master" and _openssl_available():
                    ...
                    if not sec_st["instance_cert_ok"] and _openssl_available():
                        _extra_msg += " No agent cert yet — run Test connection, then Request certificate from master."
                ...
                if role == "agent" and cfg.get("peer_master_url") and token_for_probe:
                    master_url, resolve_err = _peer_master_base_url(cfg, timeout=4)
                    if master_url:
                        master_probe = _peer_test_connection(master_url, token_for_probe)
                    else:
                        master_probe = resolve_err or "Cannot resolve master URL."
            ...
            try:
                ...
                if mode == "service" and not service_names and not service_description_filter:
                    self._reply_html(_render_setup_html(error="Service mode requires service names and/or a description filter.", ui_view="setup", ssl_warning=ssl_warning, create_mode=not edit_original_name, edit_original_name=edit_original_name or None))
                    return
