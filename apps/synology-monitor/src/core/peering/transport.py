...
``_peer_master_base_url``) intentionally stay in the monolith for now: they
also read/persist config (``_cached_peer_base_url`` writes
``cfg["peer_master_base_url"]``) and own the scheme-probe / reachability-hint
policy. They consume ``_peer_http_request`` from here (single source).
"""
