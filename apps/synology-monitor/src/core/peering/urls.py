...
mTLS-aware ``_peer_http_request`` and intentionally stay in the monolith for
now.
...
def _peer_lan_reachability_hint(master_host: str, master_port: int) -> str:
    return (
        f"If this NAS cannot reach {master_host}:{master_port}, set HOSTED_BIND_IP=0.0.0.0 on the master "
