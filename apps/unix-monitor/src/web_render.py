"""Thin web asset loader for unix-monitor (Phase 4 Slice C).

Reads templates / styles / scripts from the ``web/`` tree that ships next to
the entry script (``unix-monitor.py``). Resolution is anchored to the install
root — the directory that contains both this ``src/`` package and ``web/`` —
so it behaves the same whether the tree is run in place or installed.
...
def render_auth_shell_styles() -> str:
    """CSS for the auth shell: external files when present, else embedded copy."""
    css = load_styles(*AUTH_SHELL_STYLE_FILES)
    return css if css else _AUTH_SHELL_CSS_FALLBACK


def render_main_styles() -> str:
    """CSS for the main dashboard/setup page: external file when present, else
    embedded copy (graceful single-file fallback)."""
...
def render_auth_shell(
    *,
    ...
    the same contract as the previous inline f-string in
    ``_render_auth_shell``. Leading/trailing newlines are stripped so the output
    is byte-identical regardless of whether the template ships with a trailing
    newline.
    """
...
def render_auth_login_body() -> str:
    """Admin-password login card body: the static sign-in form plus the
    inline internet-connectivity warning script (no substitution).

    Loads ``web/templates/auth-login.html`` when present, else an embedded
    copy. Mirrors the other auth-body loaders: the leading newline + trailing
    indentation are reconstructed so the returned string is byte-identical to
    the previous inline ``body`` literal that the auth shell interpolates at
    ``{body_html}``.
    """
...
# Embedded, self-contained copy of auth-shell.css (auth :root tokens + auth
# shell rules). Kept in sync with web/styles/auth-shell.css; used only when
# that file is not deployed.
...
# Embedded, self-contained copy of parity-main.css (main dashboard styles).
# Kept in sync with web/styles/parity-main.css; used only when that file is
# not deployed.
...
# with web/templates/auth-shell.html.
