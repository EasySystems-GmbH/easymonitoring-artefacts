...
monolith during Phase 4 Slice C. These functions cover the persisted auth
state file (``unix-auth.json``), pending-setup handling, login lockout,
recovery codes, and the HMAC-signed session/challenge payloads.
...
def configure(
    *,
    runtime_data_dir: Callable[[], Path],
    product_name: str = DEFAULT_PRODUCT_NAME,
    auth_file_mode: int = 0o600,
    max_login_attempts: int = 5,
    lockout_duration_sec: int = 15 * 60,
) -> None:
    """Inject monolith-owned dependencies. Call once at startup before use."""
...
def get_auth_state_path() -> Path:
    ...
    return _runtime_data_dir_provider() / "unix-auth.json"
